...
let recentGamesAdminOnly = true; //gonna kms if someone makes this public
...
function banUser(usrid) {
  ...
  fs.writeFile("bans.txt", JSON.stringify(bans), (err) => {
    console.log(err);
  }); //even if site crashes bans won't be overrode
}
...
setInterval(() => {
  const channel = client.channels.cache.get("1360679135869009981");
  if (!channel) return;

  const gameList =
    Array.from(recentGameIds)
      .map((id) => `• ${id}`)
  ...
  if (message.content.startsWith("bb!")) {
    if (message.content.startsWith("bb!online")) {
      ...
      const messageEmbed = new discord.MessageEmbed()
        ...
        .setFooter(
          `Command made by jacskon | Last logged at ${new Date().toLocaleTimeString()}`
        );
    ...
    if (message.content.startsWith("bb!removefilter")) {
      //you need to execute the command twice the frst time it gives an error saying TypeError: filter.clearList is not a function? i am too lazy -CatHead
      if (!admins.includes(message.author.id)) {
        return;
      }
      let wordArr = message.content.split(" ");
    ...
    if (message.content.startsWith("bb!recentgames")) {
      if (recentGamesAdminOnly) {
        if (!admins.includes(message.author.id)) {
          return;
        }
      } //Dont make this fucking public
      const gameList =
        Array.from(recentGameIds)
...
function handleMessage(msg, ws, req) {
  ...
  let cleanedUsername = filter
    .censor(unicodeFilter(data.name))
    .replaceAll("*", "#");
  ...
  if (data.content.length > 5000 || data.name.length > 50) {
    return;
  }
  ...
  if (now - lastMessageTime < RATE_LIMIT_COOLDOWN_MS) {
    const remainingTime = Math.ceil(
      (RATE_LIMIT_COOLDOWN_MS - (now - lastMessageTime)) / 1000
    );
    ...
    getMsgs().then((e) => {
      ...
      const messages = e.map((message) => {
        ...
        return {
          ...
          isCommand:
            isBot && !message.embeds?.[0]?.footer?.text?.includes?.("User ID:"),
          id: message.id,
          sender: {
    ...
    if (req.body.id.length == 6) {
      res.send(
        JSON.stringify({
          success: false,
          errType: "",
          msg: "Blooket Bot doesn't work on laser tag!",
        })
      );
      return;
    }
...
function returnOnlineCount() {
  return `\n 🟢 **${
    expressWs.getWss("/onlinecount").clients.size
  }** users are on Blooket Bot`;
}
